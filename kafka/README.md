...
---|:---:|---
<a name="messageBindingObjectKey"></a>`key` | string \| [Schema Object][schemaObject] \| [AVRO Schema Object](https://avro.apache.org/docs/current/spec.html) | The message key or an object that validates the key. **NOTE**: You can also use a [Reference Object](https://asyncapi.io/docs/specifications/v2.4.0#referenceObject).<a name="messageBindingObjectSchemaIdLocation"></a>`schemaIdLocation` | string | If a Schema Registry is used when performing this operation, tells where the id of schema is stored (e.g. `header` or `payload`). | OPTIONAL | MUST NOT be specified if `schemaRegistryUrl` is not specified at the Server level
